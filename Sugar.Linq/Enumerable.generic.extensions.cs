namespace System.Linq
{
    ...
    public static partial class EnumerableExtensions
    {
        ...
        public static IEnumerable<T> Indistinct<T>(this IEnumerable<T> self)
            => self.GroupBy(x => x).SelectMany(x => x.Skip(1));
        ...
        public static T FirstOrFallback<T>(this IEnumerable<T> self, T fallback)
        {
            ...
            return enumerable.Length > 0 ? enumerable[0] : fallback;
        }
        ...
        {
            var enumerable = self as T[] ?? self.ToArray();
            return enumerable.Length > 0 ? enumerable.Last() : fallback;
        }
        ...
        {
            var enumerable = self as T[] ?? self.ToArray();
            return enumerable.Length > 0 ? enumerable.Single() : fallback;
        }
